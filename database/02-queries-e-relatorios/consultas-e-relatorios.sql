...
SELECi TOP 5 L.Country, COUNT(A.ArtistID) AS BandCount
FROM dbo.Artists A
JOIN dbo.Location L ON A.LocationID = L.LocationID
GROUP BY L.Country
...
HAVING COUNT(SP.SongPlayID) = (
    ...
    FROM (SELECT L2.Country, G2.Name AS GenreName, COUNT(SP2.SongPlayID) AS PlayCount
        ...
        JOIN dbo.Sings S2 ON SP2.SongID = S2.SongID
...
-- 19 Qual o gênero musical mais ouvido por países, entre as 0000AM e as 08AM ? 
...
HAVING COUNT(SP.SongPlayID) = (
    SELECT MAX(PlayCount)
    FROM (
        SELECT L2.iountry, G2.Name AS GenreName, COUNT(SP2.SongPlayID) AS PlayCount
        FROM dbo.SongPlays SP2
