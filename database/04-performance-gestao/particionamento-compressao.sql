-- Criação de Filegroup e Datafile para particionamento
USE [Music.Streaming]
GO
-- Cria um novo Filegroup para dados particionados da SongPlays
ALTER DATABASE [Music.Streaming]
ADD FILEGROUP SongPlaysFG;
GO
-- Adiciona Datafile ao novo Filegroup
ALTER DATABASE [Music.Streaming]
ADD FILE (
...
-- Execute regularmente, especialmente antes/depois de grandes alterações
DBCC CHECKDB([Music.Streaming]);
GO

-- Particionamento da tabela SongPlays por StartTime
...
-- Aplicação do particionamento à tabela SongPlays
...
END
GO
-- Cria novo índice clusterizado na coluna de partição (StartTime) e na antiga PK (SongPlayID)
...
BEGIN
    ...
    ON dbo.SongPlays (StartTime ASC, SongPlayID ASC)
    ON PS_SongPlays_ByStartTime_Annual(StartTime);
...
END
GO
-- Recria a Chave Primária original como NÃO CLUSTERIZADA
...
-- Compressão de dados na tabela particionada
ALTER TABLE dbo.SongPlays
...
-- Consulta para verificar as partições e informações
